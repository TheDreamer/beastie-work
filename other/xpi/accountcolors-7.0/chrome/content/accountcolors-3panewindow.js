...
var accountColors3Pane =
{
    ...
    onLoad: function()
    {
        ...
        /* Migrate option settings to Version 4.0 */
        
        if (!accountColors3Pane.prefs.getBoolPref("install-migrated"))
        {
            /* Migrate prefs to extensions.accountcolors branch */  /* compatibility with Version 3.3 and earlier */
            
            oldprefs = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("accountcolors.");
            children = oldprefs.getChildList("",{});
            children.sort();
            
            for (i = 0; i < children.length; i++)
            {
                preftype = oldprefs.getPrefType(children[i]);
                
                if (preftype == 32) accountColors3Pane.prefs.setCharPref(children[i],oldprefs.getCharPref(children[i]));
                else if (preftype == 64) accountColors3Pane.prefs.setIntPref(children[i],oldprefs.getIntPref(children[i]));
                else if (preftype == 128) accountColors3Pane.prefs.setBoolPref(children[i],oldprefs.getBoolPref(children[i]));
                else accountColors3Pane.promptService.alert(null,"Account Colors","Migration to Version 4.0 failed");
                
                if (children[i] == "thread-hidestripes") accountColors3Pane.prefs.setBoolPref("thread-showstripes",!oldprefs.getBoolPref(children[i]));
            }
            
            accountColors3Pane.prefs.setBoolPref("install-migrated",true);
        }
        
        /* Migrate option settings to Version 7.0 */
        
        if (!accountColors3Pane.prefs.getBoolPref("install-migrated70"))
        {
            accountColors3Pane.prefs.setBoolPref("compose-colorfieldbkgd",accountColors3Pane.prefs.getBoolPref("compose-colorbkgd"));
            accountColors3Pane.prefs.setBoolPref("compose-colorbkgd",false);
            accountColors3Pane.prefs.setBoolPref("compose-colorfromfont",accountColors3Pane.prefs.getBoolPref("compose-colorfrom"));
            accountColors3Pane.prefs.setBoolPref("compose-colorfrombkgd",accountColors3Pane.prefs.getBoolPref("compose-colorfrom"));
            
            accountColors3Pane.prefs.setBoolPref("install-migrated70",true);
        }
    ...
    onDragOver: function(event)
    {
        ...
        var treeBoxObject,rowHeight,eventY,orientation;
        var row = { };
        var col = { };
        var child = { };
        
        treeBoxObject = gFolderTreeView._treeElement.treeBoxObject;
        accountColorsUtilities.debugMessage(treeBoxObject,"");
        
        treeBoxObject.getCellAt(event.clientX,event.clientY,row,col,child);
        accountColorsUtilities.debugMessage(row.value,"");
        rowHeight = treeBoxObject.rowHeight;
        eventY = event.clientY-treeBoxObject.treeBody.boxObject.y-rowHeight*(row.value-treeBoxObject.getFirstVisibleRow());
    ...
    folderSetup: function()
    {
        ...
        function new_getCellProperties(row,col,props)
        {
            ...
            if (col.id == "folderNameCol")
            {
                ...
                /* add extra properties for not-dragOn, not-selected, font color, font style, font size, folder font color, no bold on unread, and show tree lines */
                /* required to select tree element styles defined in ac-3panewindow.css */
                
                if (row != accountColors3Pane.dragOnRow)
                {
                    if (!atomflag) props += " " + "not-dragOn";
                    else props.AppendElement(accountColors3Pane.atomServ.getAtom("not-dragOn"));
                }            
                
                if (!gFolderTreeView.selection.isSelected(row))
                {
                    if (!atomflag) props += " " + "not-selected";
                    else props.AppendElement(accountColors3Pane.atomServ.getAtom("not-selected"));
                }
        ...
        function new_getRowProperties(row,props)
        {
            ...
            /* add extra properties for not-dragOn, not-selected, not-focused, background color, folder background color and darker selection bar */
            ...
            {
                if (!atomflag) props += " " + "not-focused";
                else props.AppendElement(accountColors3Pane.atomServ.getAtom("not-focused"));
            }
    ...
    threadColumnHandler:
    {
        ...
        getCellProperties: function(row,col,props)
        {
            ...
            {
                /* add extra properties for not-selected, font color, font style, font size and bold subject */
                /* required to select tree element styles defined in accountcolors-3panewindow.css */
            ...
            {
                /* add extra properties for not-selected, from color, from style, from size and bold */
                /* required to select tree element styles defined in accountcolors-3panewindow.css */
            ...
            {
                /* add extra properties for not-selected, date/size/account/priority (other) color */
                /* required to select tree element styles defined in accountcolors-3panewindow.css */
        ...
        getRowProperties: function(row,props)
        {
            ...
            /* add extra properties for not-selected, not-focused, background color, show row stripes, darker selection bar */
            /* required to select tree element styles defined in accountcolors-3panewindow.css */
    ...
    cmdOptions: function()
    {
        ...
        else window.openDialog("chrome://accountcolors/content/accountcolors-options.xul","","chrome,dialog,modal,titlebar,centerscreen",null);
    }
};
