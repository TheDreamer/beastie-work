...
var accountColorsOptions =
{
    ...
    initPrefs: function()
    {
        var accounts,account,identities,identity,index,acc,id;
        var background,menulist,fontstyle,fontsize;
        var checkbox,checkstate;
        
        /* Determine Thunderbird version and set attribute */
        
        accountColorsOptions.tbVersion = "3.1";
        if (accountColorsOptions.versionComparator.compare(accountColorsOptions.appInfo.version,"5.0b1") >= 0) accountColorsOptions.tbVersion = "5.0";
        ...
        if (!accountColorsOptions.prefs.getBoolPref("install-migrated"))
        {
            ...
            for (i = 0; i < children.length; i++)
            {
                ...
                if (preftype == 32) accountColorsOptions.prefs.setCharPref(children[i],oldprefs.getCharPref(children[i]));
                else if (preftype == 64) accountColorsOptions.prefs.setIntPref(children[i],oldprefs.getIntPref(children[i]));
                else if (preftype == 128) accountColorsOptions.prefs.setBoolPref(children[i],oldprefs.getBoolPref(children[i]));
                else alert("Migration failed");
                
                if (children[i] == "thread-hidestripes") accountColorsOptions.prefs.setBoolPref("thread-showstripes",!oldprefs.getBoolPref(children[i]));
            }
            
            accountColorsOptions.prefs.setBoolPref("install-migrated",true);
        }
        ...
        for (acc = 0; acc < accountColorsUtilities.getLength(accounts) && index < 160; acc++)
        {
            ...
            {    
                for (id = 0; id < accountColorsUtilities.getLength(identities) && index < 160; id++)
                {
                    ...
                    if (id > 0)  /* not default identity */
                    {
                        ...
                        document.getElementById("accountcolors-bkgdpicker"+index).style.visibility = "hidden";
                    }
        ...
        checkbox = document.getElementById("accountcolors-compose-colorfrom");
        try
        {
            checkstate = accountColorsOptions.prefs.getBoolPref("compose-colorfrom");
            checkbox.checked = checkstate;
        }
        catch(e)
        {
        ...
        checkbox = document.getElementById("accountcolors-compose-colormenu");
        try
        {
            checkstate = accountColorsOptions.prefs.getBoolPref("compose-colormenu");
            checkbox.checked = checkstate;
        }
        catch(e)
        {
    ...
    savePrefs: function()
    {
        ...
        accountColorsOptions.prefs.setBoolPref("compose-colorfrom",document.getElementById("accountcolors-compose-colorfrom").checked);
        accountColorsOptions.prefs.setBoolPref("compose-colormenu",document.getElementById("accountcolors-compose-colormenu").checked);
    ...
    updateFontColor: function(index)
    {
        ...
        if (document.getElementById("accountcolors-accountidbox"+index).getAttribute("ac-accountidtype") == "accountid")
            for (i = index+1; i < 160 && document.getElementById("accountcolors-accountidbox"+i).getAttribute("ac-accountidtype") == "id"; i++)
            {
    ...
    pickerOpen: function(button)
    {
        ...
        if (button.id.indexOf("font") >= 0)
        {
            ...
            if (document.getElementById("accountcolors-accountidbox"+index).getAttribute("ac-accountidtype") != "id")
            {
                document.getElementById("accountcolors-picker-checkbox").hidden = false;
                document.getElementById("accountcolors-picker-checkbox").checked = accountColorsOptions.prefs.getBoolPref("picker-autobkgd");
            }
            else document.getElementById("accountcolors-picker-checkbox").hidden = true;
        }
        else
        {
            ...
            document.getElementById("accountcolors-picker-checkbox").hidden = true;
        }
    ...
    pickerUpdateElements: function(r,g,b,h,s,v)
    {
        ...
        document.getElementById("accountcolors-picker-hexstr").inputField.focus();
        try
        {
            i = document.getElementById("accountcolors-picker-hexstr").selectionEnd;
            document.getElementById("accountcolors-picker-hexstr").value = hexstr; 
        ...
        catch (e)  /* happens when called from pickerOpen() with Thunderbird 3.1 */
        {
            document.getElementById("accountcolors-picker-hexstr").value = hexstr; 
        }
    ...
    pickerPanelMouseDown: function(event)
    {
        ...
        {
            accountColorsOptions.pickerPaletteOpen = true;
    ...
    pickerPaletteShowing: function(event)
    {
        ...
        separator = document.getElementById("accountcolors-picker-separator");
    ...
    {
        if (!accountColorsOptions.pickerPaletteOpen)
        {
            document.getElementById("accountcolors-picker-panel").hidePopup();
        }
        else accountColorsOptions.pickerPaletteOpen = false;
